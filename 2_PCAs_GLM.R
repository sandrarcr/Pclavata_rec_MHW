...
#
#This script contains information on how exploratory PCAs were done as well a the linear model tested
...
#divide text to obtain factors: Individual, year and condition
factors<-matrix(NA,ncol=3,nrow=458)
for (i in 1:458) {
	factors[i,]<-strsplit(as.character(nec$X[i])," +")[[1]]
}
#Population
pop<-matrix(NA,ncol=1,nrow=458)
for (i in 1:458) {
...
#Count total data per year
nrow(nec3[nec3$year==2015,]) #77 data
nrow(nec3[nec3$year==2016,]) #78 data
nrow(nec3[nec3$year==2017,]) #76 data
...
inds<-matrix(NA,ncol=3,nrow=231)
for (i in 1:221) {
	inds[i,]<-strsplit(as.character(nec3$rep[i])," +")[[1]]
}
data.pca$ind<-as.factor(inds[,1])
#remove individuals with only one or two data
data.pca2<-data.pca[-c(229,230,231),]
...
#save resultant file
write.csv(ind_intercepts1,"Intercepts_ind.csv") #can be found on repository
